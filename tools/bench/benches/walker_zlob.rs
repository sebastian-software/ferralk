#![forbid(unsafe_code)]

use std::{
    fs,
    hint::black_box,
    path::PathBuf,
    sync::atomic::{AtomicUsize, Ordering},
    time::{SystemTime, UNIX_EPOCH},
};

use criterion::{Criterion, criterion_group, criterion_main};
use zlob::{
    ZlobFlags,
    walk::{WalkBuilder, WalkFlags},
};

static NEXT_FIXTURE: AtomicUsize = AtomicUsize::new(0);

struct Fixture {
    root: PathBuf,
}

impl Fixture {
    fn new() -> Self {
        let unique = format!(
            "ferralk-zlob-bench-{}-{}",
            std::process::id(),
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .expect("system clock is after epoch")
                .as_nanos()
                + NEXT_FIXTURE.fetch_add(1, Ordering::Relaxed) as u128
        );
        let root = std::env::temp_dir().join(unique);
        for branch in 0..16 {
            for depth in 0..4 {
                let directory = root.join(format!("branch-{branch}/depth-{depth}"));
                fs::create_dir_all(&directory).expect("create benchmark directory");
                fs::write(directory.join("match.rs"), b"fixture").expect("write matching file");
                fs::write(directory.join("skip.txt"), b"fixture").expect("write non-matching file");
            }
        }
        Self { root }
    }
}

impl Drop for Fixture {
    fn drop(&mut self) {
        let _ = fs::remove_dir_all(&self.root);
    }
}

fn walker_zlob(c: &mut Criterion) {
    let fixture = Fixture::new();

    c.bench_function("walker/zlob_parallel_filtered", |benchmark| {
        benchmark.iter(|| {
            let mut walker = WalkBuilder::new(&fixture.root).expect("benchmark root is valid");
            walker
                .options(WalkFlags::empty())
                .threads(4)
                .include("**/*.rs")
                .expect("benchmark include is valid")
                .include_flags(ZlobFlags::RECOMMENDED);
            black_box(walker.collect().expect("benchmark walk succeeds"));
        })
    });
}

criterion_group!(benches, walker_zlob);
criterion_main!(benches);
