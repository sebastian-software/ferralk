...
* **glob:** With `recursive_double_star` and `extglob`, `@(x)/**`, `!(y)/**`, and `*(a)/**` now match `x`, `x`, and `a`; `x/@(**)` matches `x`; `@(**)/y` matches `y`; and `a/@(**)/b` matches `a/b`. All of these used to be rejected. A walker exclude such as `@(a|b)/**` therefore excludes the directory `a` itself. It used to emit `a` and prune its contents, including hidden entries the exclude does not match. A subtree cover no longer prunes a directory the exclude does not match: `exclude("a/**/**")` without `match_hidden` now keeps `a` and `a/.h` instead of dropping `a/.h`.
* **glob:** with `recursive_double_star(true)`, `**` is recursive only when it forms a whole path component, that is, when an unescaped `/` or a pattern end bounds it on both sides (`**`, `**/x`, `x/**`, `x/**/y`); a `**/` hands over only at a component start. Before, any `**` run was recursive and `**/` could end inside a component, so `**/x` matched `sx` and `a/sx`, `a/**/b` matched `a/xb`, `src/**/test` matched `src/latest`, `**/.env` matched `a/x.env`, and a walker exclude `**/node_modules/**` pruned `my_node_modules`; all of these now fail to match, under `is_match`, `is_match_path`, `is_match_glob_path` and the walker. A `**` run that is not a whole component (`a**`, `**b`, `a**/b`, `**.ts`, `x@(**)`) is now ordinary stars exactly as with the option disabled: `a**/y` no longer matches `ay`, and under `is_match_glob_path` `a**/y` no longer matches `a/x/y` and `**.ts` no longer matches `src/a.ts`. A trailing `a/**/` no longer matches `a/b` without the final separator.
* **glob:** `Pattern::is_match_path` and the `filter_paths` family treat an extglob group directly after `/` as component-local, like a plain wildcard in that position: `src/@(*.ts)`, `src/+(*.ts)` and `src/!(x)` no longer accept `src/a/b.ts`, as `src/*.ts` does not. For `+(…)`/`*(…)` only the first iteration is in that position, so `src/+(*.ts)` still accepts `src/a.tsb/c.ts` like `src/*.ts*.ts`. Brace alternatives are judged independently: `{src/@(*.ts),zz}` and `{src/@(*.ts),lib/*}` now agree. Under both `is_match_path` and `is_match_glob_path`, and therefore the default `Walker`, an extglob reads like the same text outside a group. `**/*.@(ts|js)` matches at every depth instead of stopping one directory deep. A separator or recursive `**` written inside a group crosses components (`*/@(a/b)` accepts `x/a/b`, `a/@(x|**)/*.ts` accepts `a/b/c/d.ts`). An outer star run such as `***` behind `/` crosses like `xy/***/z`, and without `recursive_double_star` `src/**/*.@(ts|js)` accepts `src/a/b/c.ts` like `src/**/*.ts`. A wildcard behind a literal inside a group crosses again, as in `a/b*`. `is_match` is unchanged.
* **walker:** `Walker::include`, `Walker::exclude` and their `try_` forms now return a `PatternError` for a relative pattern or brace alternative that starts with `!` not followed by `(` (for example `!src/**`, `!*.log`, `{!a,b}/**`), at the byte offset of that `!`. Previously such a pattern compiled as a literal `!` and silently selected nothing. Move the pattern without the `!` to `exclude()`, or write `\!` for a literal `!`; `!(…)` extglobs, interior `!`, and absolute patterns whose `!` lies below the walk root are unchanged.
* **walker:** Walker includes and excludes ignore one leading `./` on every brace-expanded alternative, exactly as `Pattern::is_match_glob_path` does. `{./src/*.rs,lib/*.rs}` used to be accepted and select only `lib/*.rs` without an error; it now selects from both directories. `{./src/*.rs,./lib/*.rs}` and `{./a.rs}` used to be rejected with "a walker-relative pattern with a `.` component is not normalized"; they are now accepted and select what the matcher matches. A `./` that does not start an expanded alternative (`src/{./a,./b}/*.rs`) is still rejected as a `.` component.
* **glob:** with `extglob(true)` and `match_hidden` off, a negated extglob no longer selects a hidden component that it reaches by crossing a separator. Before, `!(x)` and `a!(x)` matched `a/.env` under `is_match` and `is_match_path`, `src/!(x)` matched `src/a/.env`, `!(.env)` matched `a/.git`, and `!(x).env` matched `a/.env`. All of these now return false, the same result as `*`, `a*`, `src/*`, and `*.env`. Enable `match_hidden` to select hidden components.
* **walker:** A walk root with a `..` component, combined with an absolute include or exclude pattern, now always fails with "an absolute pattern needs a walk root without a `..` component". Before, a pattern that diverged from the root's spelling before the `..` was accepted and selected nothing, so the walk returned Ok with zero entries. Examples are `$B/b/**` or `$B/zzz/**` under `$B/a/../b`, or another drive or share on Windows. This applies to `include`, `exclude`, `add_root`, `add_roots`, and their `try_` forms. Relative patterns under such a root keep working.

### Features

* **glob:** add PatternOptions::walker() preset for the walker dialect ([#409](https://github.com/sebastian-software/ferralk/issues/409)) ([d1de0eb](https://github.com/sebastian-software/ferralk/commit/d1de0ebe4b061cc6e8e264e2f0e886346d6cc84a)), closes [#399](https://github.com/sebastian-software/ferralk/issues/399)


### Bug Fixes

* **glob:** keep extglob groups behind a separator component-local in path filters ([#415](https://github.com/sebastian-software/ferralk/issues/415)) ([373c803](https://github.com/sebastian-software/ferralk/commit/373c8033a07e824717ec8ce8c23ed234830dab86)), closes [#393](https://github.com/sebastian-software/ferralk/issues/393)
* **glob:** keep negated extglobs out of hidden components ([#408](https://github.com/sebastian-software/ferralk/issues/408)) ([33be65a](https://github.com/sebastian-software/ferralk/commit/33be65ad83e400f2f43c38ad8cd00803c029ec90)), closes [#394](https://github.com/sebastian-software/ferralk/issues/394)
* **glob:** make `**` recursive only as a whole path component ([#420](https://github.com/sebastian-software/ferralk/issues/420)) ([b1f42e6](https://github.com/sebastian-software/ferralk/commit/b1f42e608f5843d25d0344b1dfe604423f6ae1e9)), closes [#419](https://github.com/sebastian-software/ferralk/issues/419)
* **glob:** read extglob groups beside a globstar like their inlined alternatives ([#423](https://github.com/sebastian-software/ferralk/issues/423)) ([ac721ae](https://github.com/sebastian-software/ferralk/commit/ac721ae418aa34d16c1d1126c7a482ddc7306475)), closes [#422](https://github.com/sebastian-software/ferralk/issues/422)
...
* **walker:** apply excludes inside hidden directories ([#425](https://github.com/sebastian-software/ferralk/issues/425)) ([fc209ab](https://github.com/sebastian-software/ferralk/commit/fc209ab921d425b9a8c729e4c89a9f3528403439)), closes [#424](https://github.com/sebastian-software/ferralk/issues/424)
* **walker:** ignore a leading `./` on every brace alternative ([#413](https://github.com/sebastian-software/ferralk/issues/413)) ([dba4c4b](https://github.com/sebastian-software/ferralk/commit/dba4c4ba21cf82f73bc3d07fffb3a0490d9a7727)), closes [#395](https://github.com/sebastian-software/ferralk/issues/395)
* **walker:** mark walk results `#[must_use]` and document silent footguns ([#416](https://github.com/sebastian-software/ferralk/issues/416)) ([885ae13](https://github.com/sebastian-software/ferralk/commit/885ae13e05b8ee6c90b05e75d051dd1dc6e0d8b2)), closes [#398](https://github.com/sebastian-software/ferralk/issues/398)
* **walker:** print a WalkError's io::Error cause once ([#411](https://github.com/sebastian-software/ferralk/issues/411)) ([cb38857](https://github.com/sebastian-software/ferralk/commit/cb38857175402fc2d690e0ffaa889c3b036284bc)), closes [#400](https://github.com/sebastian-software/ferralk/issues/400)
* **walker:** reject a leading `!` in include and exclude patterns ([#417](https://github.com/sebastian-software/ferralk/issues/417)) ([35dff8d](https://github.com/sebastian-software/ferralk/commit/35dff8dc25e0dae2e52473611016011b55163622)), closes [#397](https://github.com/sebastian-software/ferralk/issues/397)
* **walker:** reject every absolute pattern under a walk root with `..` ([#407](https://github.com/sebastian-software/ferralk/issues/407)) ([d11f7d3](https://github.com/sebastian-software/ferralk/commit/d11f7d339c74d9cd8786f09a195d26d319477e00)), closes [#396](https://github.com/sebastian-software/ferralk/issues/396)


### Performance Improvements

* **glob:** prefilter on a required interior literal ([#418](https://github.com/sebastian-software/ferralk/issues/418)) ([e401b7e](https://github.com/sebastian-software/ferralk/commit/e401b7eb2460b482a4a670506aae376a3eef95e2)), closes [#403](https://github.com/sebastian-software/ferralk/issues/403)
* **walker:** sort walk results by bytes below the root ([#410](https://github.com/sebastian-software/ferralk/issues/410)) ([8944ace](https://github.com/sebastian-software/ferralk/commit/8944ace0e6b8fc0dcc097d7daf0c1da324356eaa)), closes [#404](https://github.com/sebastian-software/ferralk/issues/404)
