...
| Shape of a returned path | – | Relative to `cwd`: `src/lib.rs`. | The root joined with the relative path. | The root joined with the relative path: `Walker::new(".")` yields `./src/lib.rs`, `Walker::new("src")` yields `src/lib.rs`. `entry.path().strip_prefix(entry.root())` is the relative part. |
