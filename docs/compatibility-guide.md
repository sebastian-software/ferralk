...
| Several patterns at once | `globset::GlobSet`, with `matches` for the indices. | An array of patterns. | `OverrideBuilder`. | `Walker`: one `include` or `exclude` call per pattern; includes are OR-ed. `ferralk-glob`: `PatternSet::new(globs, PatternOptions::walker())?`; its error's `index()` names the failed glob. `is_match_glob_path` reads patterns like a `GlobSet` built with `literal_separator(true)`, `is_match` like an unconfigured one, and `matches_glob_path_into(path, &mut vec)` takes the place of `GlobSet::matches_into`. Collect compiled `Pattern`s for per-glob options. |
