...
| `ferralk-glob` 0.1 publication | `cargo package` and `cargo publish --dry-run` completed locally; the release workflow validates the locked workspace and publishes it first. | Configure crates.io trusted publishing for this public repository, then approve a release PR. |
| `ferralk` 0.1 publication | The same logical release increments both crates and its exact `ferralk-glob` dependency; the publish workflow orders it after `ferralk-glob`. | The preceding `ferralk-glob` publication must complete, then crates.io trusted publishing may publish `ferralk`. |
