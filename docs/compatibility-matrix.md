...
| `ZLOB_FOLLOW_SYMLINKS` | `WalkOptions::follow_symlinks` | Implemented (M2) | Default off; canonical-path cycle guard when enabled. The guard is per root traversal, so duplicate, overlapping and alias roots remain independent. |
