...
| `test_posix.zig`, `test_append.zig`, `test_c_api.c`, `test_errfunc.zig` | No direct equivalent | C result buffers, offsets, append, callbacks, alternate directory callbacks, tilde expansion, and POSIX runtime behavior are deliberately excluded by ADR-0003. |
