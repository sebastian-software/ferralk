...
| `test_walk.zig` | Walker fixtures for filtering, depth, kind, basename, symlinks, Gitignore allowlisting, serial/parallel equivalence | Public Rust-shaped traversal contract is being ported incrementally; visitor callbacks and C error callbacks have no equivalent API. |
