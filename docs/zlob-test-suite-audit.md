...
| `test_rust_glob.zig` | Walker regression fixture for literal, wildcard, class, nested, special-character, `./`, and trailing-slash patterns | Covered for root-relative traversal filtering; zlob's C-shaped result buffer plus `.`/`..` root entries remain outside the Walker API. |
