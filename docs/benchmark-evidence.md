...
| Retained pattern memory | Upper bounds on the bytes a compiled pattern keeps, from a literal to a 64-alternative brace, a `PatternSet` of 100 globs, and a walker with 100 includes and 100 excludes over one root and over four | [`pattern_memory_regression.rs`](../crates/ferralk/tests/pattern_memory_regression.rs), every platform test | Yes |
...
| Walker, wall time | Warm-cache traversal of synthetic trees, including serial, parallel, `stream()`, and `stream_parallel()` over the 53k-file repository shape, a deep chain, nested Git-ignore rules with negations, include-plus-covering-exclude pruning, and comparisons with `ignore` parallel | [`walker-bench.yml`](../.github/workflows/walker-bench.yml), every pull request compares the merge base and head back to back in one job for every shipped backend; medians and head/base ratios are published in the job summary and as artifacts | No |
